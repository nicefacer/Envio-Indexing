...
let createEntityRelationship = async (
  ...
  let derivedFromTo =
    derivedFromFieldKey != "" ? `"id": "${derivedFromFieldKey}"` : `"${relationalKey}" : "id"`
...
let trackAllTables = async () => {
  ...
  let _ = await trackTable(~tableName="awesomeEntity")
  let _ = await createSelectPermissions(~tableName="awesomeEntity")
}
